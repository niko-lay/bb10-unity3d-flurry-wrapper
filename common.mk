...
LDFLAGS+=-marm, -Wl,-z,relro -Wl,-z,now $(if $(filter g so shared,$(VARIANTS)),,-pie)
...
##################################################################################################################
### Please add the following lines to link against Flurry library assuming it resides in $(PROJECT_ROOT)/Flurry #
##################################################################################################################
EXTRA_INCVPATH+=$(PROJECT_ROOT)/Flurry
#Extra library search path for target overrides and patches 
EXTRA_LIBVPATH+=$(QNX_TARGET)/$(CPUVARDIR)/usr/lib/qt4/lib $(PROJECT_ROOT)/Flurry/$(CPUVARDIR)
LIBS+= cpp QtCore curl socket crypto packageinfo screen bbdevice Flurry
##################################################################################################################
